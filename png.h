...
uint8_t *read_png(const char* path, size_t width, size_t height) {
    png_t png;
    bool valid = true;
    ...
    if (valid) {
        buf = (uint8_t*)calloc(png.width * png.height * png.bpp + png.height, sizeof(uint8_t));
...
bool write_png(const uint8_t *buf, size_t width, size_t height, const char* path) {
    png_t png;
    bool valid = true;
    ...
    err = png_set_data(&png, width, height, 8, PNG_GREYSCALE, (unsigned char *)buf);
